...
matrix = Path("docs/parity/python_polars_full_matrix.md").read_text()
body = matrix.split("## Top-30")[0]

rows = re.findall(r"\| `([^`]+)` \| .*? \| (реализовано|не реализовано) \| (—|high|medium|low) \|", body)
